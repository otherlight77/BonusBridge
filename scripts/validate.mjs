...
const codeFiles=[];async function collect(directory){for(const entry of await readdir(directory,{withFileTypes:true})){if(entry.name==='.git')continue;const path=join(directory,entry.name);if(entry.isDirectory())await collect(path);else codeFiles.push(path)}}await collect('.');
...
for(const file of['manifest.webmanifest','offline.html','robots.txt','sitemap.xml','sw.js','.nojekyll','.github/workflows/deploy-pages.yml','assets/icons/logo.svg','assets/icons/logo-light.svg','assets/icons/logo-dark.svg','assets/icons/favicon.svg','assets/icons/app-icon.svg','assets/social-card.svg'])await access(file);

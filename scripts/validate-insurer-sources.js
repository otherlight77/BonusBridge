...
const names = ['insurers','insurer-sources','recognition-rules','documents','provinces-canada','regions-france','scraping-status','source-history'];
...
for (const country of ['FR','CA']) {
  ...
  if (records.length !== 3) failures.push(`${country}: exactement trois groupes sont requis.`);
...
for (const source of data['insurer-sources'].records) if (!source.url || !source.type || !source.lastCheckedAt) failures.push(`${source.id}: source incomplète.`);
